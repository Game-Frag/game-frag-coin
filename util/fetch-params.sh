...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/GAMEFRAGParams"
    else
        PARAMS_DIR="$HOME/.gamefrag-params"
    fi
fi
...
Failed to fetch the GAMEFRAG zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
GAMEFRAG - fetch-params.sh

This script will fetch the GAMEFRAG zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common GAMEFRAG zkSNARK parameters. Note that it is
