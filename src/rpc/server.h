...
/**
 * GameFrag RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZfragSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzfragseed(const UniValue& params, bool fHelp);
extern UniValue getzfragseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzfrag(const UniValue& params, bool fHelp);
extern UniValue dzfragstate(const UniValue& params, bool fHelp);
