...
public:
    ...
    bool WriteZFRAGSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZFRAGSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZFRAGSeed_deprecated(uint256& seed);
    bool EraseZFRAGSeed();
    bool EraseZFRAGSeed_deprecated();

    bool WriteZFRAGCount(const uint32_t& nCount);
    bool ReadZFRAGCount(uint32_t& nCount);
