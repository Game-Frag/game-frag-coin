                                      ...
                                      std::string& strError)
{
    ...
    if (txOut.nValue != consensus.nMNCollateralAmt) {
        strError = strprintf("Invalid collateral tx value, must be %s FRAG", FormatMoney(Params().GetConsensus().nMNCollateralAmt));
...
bool CWallet::CreateCoinstakeOuts(const CFragStake& stakeInput, std::vector<CTxOut>& vout, CAmount nTotal) const
{
