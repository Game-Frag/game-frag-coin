...
// The zFRAG block index is the first appearance of the accumulator checksum that was used in the spend
...
bool CZCrctStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zFRAG staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
{
    //The unique identifier for a zFRAG is a hash of the serial
...
{
    //Create an output returning the zFRAG that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZFRAGOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zFRAG output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zFRAG", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZFRAGOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zFRAG output", __func__);
...
{
    CzFRAGTracker* zfragTracker = pwallet->zfragTracker.get();
    CMintMeta meta;
    if (!zfragTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zfragTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
