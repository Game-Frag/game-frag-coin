...
const char * const GAMEFRAG_CONF_FILENAME = "gamefrag.conf";
const char * const GAMEFRAG_PID_FILENAME = "gamefrag.pid";
const char * const GAMEFRAG_MASTERNODE_CONF_FILENAME = "masternode.conf";


// GAMEFRAG only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\GAMEFRAG
// Windows >= Vista: C:\Users\Username\AppData\Roaming\GAMEFRAG
// Mac: ~/Library/Application Support/GAMEFRAG
// Unix: ~/.gamefrag
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GAMEFRAG";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GAMEFRAGParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GAMEFRAGParams
    // Mac: ~/Library/Application Support/GAMEFRAGParams
    // Unix: ~/.gamefrag-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GAMEFRAGParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "GAMEFRAGParams";
#else
    // Unix
    return pathRet / ".gamefrag-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/gamefrag/sapling-spend.params";
        sapling_output = "/usr/share/gamefrag/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/gamefrag/sapling-spend.params";
            sapling_output = "/usr/local/share/gamefrag/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", GAMEFRAG_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", GAMEFRAG_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty gamefrag.conf if it does not exist
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override gamefrag.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", GAMEFRAG_PID_FILENAME));
