...
SettingsFaqWidget::SettingsFaqWidget(GAMEFRAGGUI* parent, MNModel* mnModel) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendableFRAG,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableFRAG,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableFRAG,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("GAMEFRAG is a form of digital online money using blockchain technology "
        ...
        formatFAQParagraph(
            tr("GAMEFRAG utilizes a Proof of Stake (PoS) consensus system algorithm, "
    ...
    QString unspendableFRAGContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received GAMEFRAG requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your GAMEFRAG wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableFRAG->setText(unspendableFRAGContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of GAMEFRAG with a minimum of 600 confirmations.")) +
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.game-frag.com'>" + tr("#support in our Discord") + "</a>.")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(mnModel->getMNCollateralRequiredAmount(), BitcoinUnits::FRAG)) +
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in GAMEFRAG Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the GAMEFRAG network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableFRAG, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableFRAG);});
    ...
    if (parent)
        connect(parent, &GAMEFRAGGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
