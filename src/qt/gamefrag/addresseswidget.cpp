...
#include "qt/gamefrag/forms/ui_addresseswidget.h"
#include "qt/gamefrag/addresslabelrow.h"
#include "qt/gamefrag/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(GameFragGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter FRAG address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress fragAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(fragAdd.Get(), label.toUtf8().constData(),
                fragAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
