...
#include "qt/gamefrag/forms/ui_send.h"
#include "qt/gamefrag/addnewcontactdialog.h"
#include "qt/gamefrag/qtutils.h"
#include "qt/gamefrag/sendchangeaddressdialog.h"
#include "qt/gamefrag/optionbutton.h"
#include "qt/gamefrag/sendconfirmdialog.h"
#include "qt/gamefrag/myaddressrow.h"
#include "qt/gamefrag/guitransactionsutils.h"
...
SendWidget::SendWidget(GameFragGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("FRAG");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zFRAG");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (FRAG) or private coins (zFRAG)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("GameFrag address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 FRAG");
    ...
    setCssProperty(coinIcon, "coin-icon-frag");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onFRAGSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onFRAGSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send FRAG");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zFRAG");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZfrag = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZfrag));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZfrag ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendCrct ? AskPassphraseDialog::Context::Send_FRAG : AskPassphraseDialog::Context::Send_zFRAG, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendCrct) ? send(recipients) : sendZfrag(recipients)) {
...
bool SendWidget::sendZfrag(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zFRAG is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zFRAG selector if applicable
    ...
    if (walletModel->sendZfrag(
        ...
        inform(tr("zFRAG transaction sent!"));
        ZCrctControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZFRAG_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zFRAG require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zFRAG transaction
        ...
        emit message("zFRAG transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isFRAG){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zFRAG in your balance to select."));
        }
    }
}
...
void SendWidget::onFRAGSelected(bool _isFRAG){
    isFRAG = _isFRAG;
    setCssProperty(coinIcon, _isFRAG ? "coin-icon-frag" : "coin-icon-zfrag");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress fragAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(fragAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(fragAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
