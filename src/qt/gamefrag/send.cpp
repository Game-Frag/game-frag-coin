...
#include "qt/gamefrag/loadingdialog.h"
    ...
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a GAMEFRAG URI"));
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent FRAGs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield FRAGs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent FRAGs to shield."));
    }
}
...
void SendWidget::onFRAGSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto fragAdd = Standard::DecodeDestination(address.toStdString(), isStaking, isExchange, isShielded);

        if (!Standard::IsValidDestination(fragAdd) || isStaking) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(fragAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
