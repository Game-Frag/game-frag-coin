...
#include "qt/gamefrag/forms/ui_coldstakingwidget.h"
#include "qt/gamefrag/qtutils.h"
#include "amount.h"
#include "guiutil.h"
#include "qt/gamefrag/requestdialog.h"
...
ColdStakingWidget::ColdStakingWidget(GameFragGUI* parent) :
    PWidget(parent),
    ...
    isLoading(false)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your FRAGs, letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Accept FRAG delegation / Delegate FRAG"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select FRAG outputs to delegate.");
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 FRAG" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
