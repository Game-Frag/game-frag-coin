...
#include "qt/gamefrag/forms/ui_privacywidget.h"
#include "qt/gamefrag/qtutils.h"
#include "guiutil.h"
#include "qt/gamefrag/denomgenerationdialog.h"
#include "qt/gamefrag/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(GameFragGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zFRAG anonymizes your FRAG by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zFRAG or convert back to FRAG"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 FRAG ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zFRAG");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zFRAG");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zFRAG");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zFRAG");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zFRAG");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zFRAG");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zFRAG");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zFRAG");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zFRAG Movements"));
    ...
    // Only Convert to FRAG enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzFRAG->setTitleClassAndText("btn-title-grey", "Total 0 zFRAG");
    ui->btnTotalzFRAG->setSubTitleClassAndText("text-subtitle", "Show denominations of zFRAG owned.");
    ui->btnTotalzFRAG->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select FRAG outputs to mint into zFRAG.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zFRAG");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzFRAG, SIGNAL(clicked()), this, SLOT(onTotalZfragClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zFRAG");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of FRAG to mint into zFRAG"));
    }else{
        btnText = tr("Convert back to FRAG");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zFRAG to convert back into FRAG"));
    }
...
void PrivacyWidget::onTotalZfragClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzFRAG->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzFRAG->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zFRAG is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zFRAG, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zFRAG").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zFRAG minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZfrag(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zFRAG converted back to FRAG"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any FRAG to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zFRAG </b>";
    ...
    ui->btnTotalzFRAG->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
