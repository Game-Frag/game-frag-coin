...
#include "qt/gamefrag/dashboardwidget.h"
#include "qt/gamefrag/forms/ui_dashboardwidget.h"
...
DashboardWidget::DashboardWidget(GameFragGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of FRAG staked and/or generated from Masternodes."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareCrct, "square-chart-frag");
    setCssProperty(ui->labelSquareMNRewards, "square-chart-mnrewards");
    setCssProperty(ui->labelCrct, "text-chart-frag");
    ...
    ui->labelAmountMNRewards->setText("0 FRAG");
    ui->labelAmountCrct->setText("0 FRAG");
    setCssProperty(ui->labelAmountCrct, "text-stake-frag-disable");
    ...
    ui->btnHowTo->setText(tr("How to get FRAG"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("FRAG")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isCrct = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZFRAG && modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        chartData->valuesCrct.append(frag);
        chartData->valuesMNRewards.append(mnrewards);

        int max = std::max(frag, mnrewards);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("FRAG");
    set1 = new QBarSet("FRAG MN");
    ...
    if (chartData->totalCrct > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountCrct, "text-stake-frag");
        setCssProperty(ui->labelAmountMNRewards, "text-mnrewards-mnrfrag");
    } else {
        setCssProperty(ui->labelAmountCrct, "text-stake-frag-disable");
        setCssProperty(ui->labelAmountMNRewards, "text-stake-mnrewards-disable");
    }
    ...
    ui->labelAmountMNRewards->setText(QString::number(chartData->totalMNRewards / COIN) +" FRAG MNR");
