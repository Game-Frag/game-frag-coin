...
#include "qt/gamefrag/forms/ui_snackbar.h"
#include "qt/gamefrag/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(GAMEFRAGGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &GAMEFRAGGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
