...
#include "ui_zfragcontroldialog.h"
    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zFRAG Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zFRAG.";
            else if (!mint.isSeedCorrect)
                strReason = "The zFRAG seed used to mint this zFRAG is not the same as currently hold in the wallet";
            else
