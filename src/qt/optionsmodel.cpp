...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::FRAG);
    ...
    if (settings.contains("nAnonymizeGamefragAmount") || reset)
        gArgs.SoftSetArg("-anonymizegamefragamount", settings.value("nAnonymizeGamefragAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in gamefrag.cpp during shutdown to also remove the window positions
