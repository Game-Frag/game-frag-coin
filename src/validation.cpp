...
/** Fees smaller than this (in ufrag) are considered zero fee (for relaying, mining and transaction creation)
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zFRAG mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zFRAG mint output in tx %s",
...
{
    util::ThreadRename("gamefrag-scriptch");
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        ...
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nGamefragBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nGamefragBadBlockBits)) {
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        if (isBlockFromFork) {
            ...
            if(hasZFRAGInputs) {
                for (const CTxIn& zFragInput : zFRAGInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zFragInput);
