...
CzFRAGWallet::CzFRAGWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zfrag seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZFRAGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFRAGSeed_deprecated()) {
                    LogPrintf("%s: Updated zFRAG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zfrag seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zfrag wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzFRAGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZFRAGCount(nCountLastUsed);
    else if (!walletdb.ReadZFRAGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzFRAGWallet::Lock()
{
    seedMaster = 0;
}

void CzFRAGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzFRAGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFRAG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzFRAGWallet::LoadMintPoolFromDB()
{
...
void CzFRAGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzFRAGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzFRAGWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zfragTracker->HasPubcoinHash(pMint.first)) {
...
bool CzFRAGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFRAG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zfragTracker which also adds to database
    pwalletMain->zfragTracker->Add(dMint, true);
...
void CzFRAGWallet::SeedToZFRAG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzFRAGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzFRAGWallet::GenerateDeterministicZFRAG(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzFRAGWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzFRAGWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzFRAGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
