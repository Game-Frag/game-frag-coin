...
bool CLegacyZFragStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZFragStake from non zc-spend", __func__);
    ...
    *this = CLegacyZFragStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zfrag stake origin", __func__);

    // All good
    return true;
}

CLegacyZFragStake::CLegacyZFragStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZFragStake::GetIndexFrom() const
{
...
CAmount CLegacyZFragStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZFragStake::GetUniqueness() const
{
...
bool CLegacyZFragStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zFRAG stake block: height %d outside range", __func__, nHeight);
