...
CzFRAGTracker::CzFRAGTracker(std::string strWalletFile)
{
...
CzFRAGTracker::~CzFRAGTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzFRAGTracker::Init()
{
...
bool CzFRAGTracker::Archive(CMintMeta& meta)
{
...
bool CzFRAGTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzFRAGTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzFRAGTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzFRAGTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzFRAGTracker::GetSerialHashes()
{
...
CAmount CzFRAGTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzFRAGTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzFRAGTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzFRAGTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzFRAGTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzFRAGTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzFRAGTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzFRAGTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzFRAGTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzFRAGTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzFRAGTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzFRAGWallet* zFRAGWallet)
{
    bool iszFRAGWalletInitialized = (NULL != zFRAGWallet);
    CMintMeta meta;
    ...
    if (! iszFRAGWalletInitialized)
        zFRAGWallet = new CzFRAGWallet(strWalletFile);
    meta.isSeedCorrect = zFRAGWallet->CheckSeed(dMint);
    if (! iszFRAGWalletInitialized)
        delete zFRAGWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzFRAGTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzFRAGTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzFRAGTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzFRAGTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzFRAGTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzFRAGTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzFRAGWallet* zFRAGWallet = new CzFRAGWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zFRAGWallet);
        }
        delete zFRAGWallet;
        LogPrint("zero", "%s: added %d dzfrag from DB\n", __func__, listDeterministicDB.size());
    }
