...
class ReorgStakeTest(GamefragTestFramework):
    ...
    def check_money_supply(self, expected_frag):
        # verify that nodes have the expected FRAG supply
        frag_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(frag_supply, [DecimalAmt(expected_frag)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that FRAG supply was properly updated after the reorgs (including burned fee)
