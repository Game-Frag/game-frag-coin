...
class ZerocoinSpendTest(GamefragTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zfrag_bal, frag_bal):
            zfrag_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zfrag_bal)
            frag_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], frag_bal)
            return zfrag_bal, frag_bal
        ...
        zfrag_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zfrag_balance, 6666)
        ...
        zfrag_balance, balance = check_balances(denom_0, zfrag_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zfrag_balance, balance = check_balances(denom_2, zfrag_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zfrag_balance, balance = check_balances(denom_3, zfrag_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zfrag",
        ...
        _, _ = check_balances(denom_1, zfrag_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
