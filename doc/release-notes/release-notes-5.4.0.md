GAMEFRAG Core version v5.4.0 is now available from: <https://github.com/gamefrag-project/gamefrag/releases>
...
The `autocombinerewards` RPC command was soft-deprecated in v5.3.0 and replaced with explicit setter/getter commands `setautocombinethreshold`/`getautocombinethreshold`. GAMEFRAG Core, by default, will no longer accept the `autocombinerewards` command, returning a deprecation error, unless the `gamefragd`/`gamefrag-qt` is started with the `-deprecatedrpc=autocombinerewards` option.
...
- #2579 gamefrag-cli better error handling + libevent RAII upstream backports (furszy)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/gamefrag-project-translations/), and every single person who tested the release candidates.
