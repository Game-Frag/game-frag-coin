...
*Setup instructions for a gitian build of GAMEFRAG Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the GAMEFRAG
...
to the GAMEFRAG GitHub release page.
...
- [Building GAMEFRAG Core](#building-gamefrag-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/gamefrag-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/gamefrag-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/gamefrag-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/gamefrag-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/gamefrag-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/gamefrag-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/gamefrag-project/gitian.sigs/) repository:
...
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/gamefrag-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/gamefrag-osx-*-build.assert
```
